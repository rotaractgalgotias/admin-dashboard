...
export function AdaptiveDashboardComponent() {
  ...
  return (
    <div className={`flex-1 space-y-4 p-4 md:p-8 ${isDarkMode ? 'dark' : ''}`}>
            ...
            <div className="space-y-4">
              {mockData.activityLog.map((activity) => (
                <div key={activity.id} className="flex items-center space-x-4">
                  {activity.action === 'CREATE' && <CheckCircle2 className="h-5 w-5 text-green-500" />}
                  {activity.action === 'UPDATE' && <AlertCircle className="h-5 w-5 text-blue-500" />}
